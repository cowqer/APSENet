...
class Sobel7x7(nn.Module):
    """7x7 Sobel 卷积（保持尺寸不变 padding=3）"""
    def __init__(self, in_channels, out_channels):
        super().__init__()
        ...
        kernel = torch.stack([sobel_kernel_x, sobel_kernel_y])  # [2,7,7]
        kernel = kernel.unsqueeze(1)
        kernel = kernel.repeat(in_channels, 1, 1, 1)  # [2*in_ch,1,7,7]

        self.weight = nn.Parameter(kernel, requires_grad=False)

        self.conv = nn.Conv2d(
            in_channels, out_channels, kernel_size=7,
            stride=1, padding=3, bias=False, groups=in_channels
        )
        self.conv.weight.data = self.weight[:out_channels]
...
class SSblock33(SEA):
    def __init__(self, dim):
        super(SSblock33, self).__init__(dim)
        # self.sobel1 =  Sobel5x5(dim, dim)
        self.sobel2 = Sobel3x3(dim, dim)      
        
class SSblock57(SEA):
    def __init__(self, dim):
        super(SSblock57, self).__init__(dim)
        self.sobel1 =  Sobel5x5(dim, dim)
        self.sobel2 = Sobel7x7(dim, dim)
                 
                 
class SSblock37(SEA):
    def __init__(self, dim):
        super(SSblock37, self).__init__(dim)
        # self.sobel1 =  Sobel5x5(dim, dim)
        self.sobel2 = Sobel7x7(dim, dim)             
...
#     y2 = block1(x)
#     y3 = block2(x)
#     print(y1.shape)  # torch.Size([2, 64, 128, 128])    
#     print(y2.shape)  # torch.Size([2, 64, 128, 128])
    # print(y3.shape)  # torch.Size([2, 64, 128, 128])
