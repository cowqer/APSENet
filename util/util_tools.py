...
class MyOptim(object):
    ...
    def cosine_scheduler(base_lr, cur_iter, max_iter, warmup_iter=0):
        """Return lr according to Cosine Annealing with optional Warmup."""
        if cur_iter < warmup_iter:
            # Linear warmup
            return base_lr * float(cur_iter) / float(max(1, warmup_iter))

        # Cosine decay
        progress = float(cur_iter - warmup_iter) / float(max(1, max_iter - warmup_iter))
    ...
    def adjust_learning_rate_poly_no_backbone(args, optimizer, iter, max_iter):
        ...
        lr = MyOptim.cosine_scheduler(args.lr, iter, max_iter, warmup_iter=args.warmup)
    ...
    def get_finetune_optimizer_backbone(args, model):
        ...
        if args.is_sgd:
            opt = optim.SGD([{'params': pretrain_weight_list, 'lr': lr / lr_ratio_list[0]},# lr/1000
                             {'params': pretrain_bias_list, 'lr': lr / lr_ratio_list[1]},# lr/500
                             {'params': weight_list, 'lr': lr * lr_ratio_list[2]},# lr x 1
                             {'params': bias_list, 'lr': lr * lr_ratio_list[3]}], momentum=0.90, weight_decay=0.0005)# lr x 2
        else:
...
class AverageMeter:
    ...
    def __init__(self, dataset, device=None):
        ...
        if self.benchmark == 'isaid':
            ...
            self.nclass = 15
        elif self.benchmark == 'dlrsd_mining':
            self.nclass = 18
