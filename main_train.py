...
class Runner(object):
    ...
    def __init__(self, args):
        ...
        # 动态选择网络
        net_cls = globals()[self.args.net_name] if hasattr(self.args, "net_name") else APSENetwork
        ...
        shutil.copy(os.path.join("net", "net_tools_pro.py"), os.path.join(args.loggerpath, "net_tools_pro.py"))
        # 记录模型结构到日志
    ...
    def train(self):
        ...
        for epoch in range(self.args.epoch_num):
            ...
            Logger.info(f"[Epoch {epoch}] Learning Rate: {now_lr:.7f}")
            # ★ 写入 TensorBoard
            Logger.tbd_writer.add_scalar("data/lr", now_lr, epoch)
